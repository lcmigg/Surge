...
console.log("vipa: " + vipa);

if ($request.url && vipa && $request.url.indexOf(vipa) !== -1) {
...
if ($request.url && vipb && $request.url.indexOf(vipb) !== -1) {
  chxm1023.data.ar_property["psnl_vip_property"] = vip;
}

if ($request.url && tqzx && $request.url.indexOf(tqzx) !== -1) {
  chxm1023.data.data = {
    "document": [
