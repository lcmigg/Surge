/** 
☑️ 资源解析器 ©𝐒𝐡𝐚𝐰𝐧  ⟦2021-03-20 00:00⟧
----------------------------------------------------------
...
function TagCheck_QX(content) {
  console.log(content)
    ...
    if (duplist.length >= 1) {
      ...
      if (Pdel==0){
        $notify("引用" + "⟦" + subtag + "⟧" + " 内有" + no + "个名字重复的节点 ", "已添加⌘符号作为区分:", " ⨁ " + duplist.join("\n ⨁ "))
      } else {
        $notify("引用" + "⟦" + subtag + "⟧" + " 内有" + no + "个名字重复的节点 ", "已全部删除处理，如需保留请添加参数 del=0:", " ⨁ " + duplist.join("\n ⨁ "))
      }
    }
    return Nlist
}
